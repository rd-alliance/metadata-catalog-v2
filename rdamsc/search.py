...
def scheme_search():
    ...
    if request.method == "POST" and form.validate():
        ...
        if title_wild:
            ...
            for m in Scheme.search(Q.versions.any(Q.title.search(title_query))):
                sub_results_by_id[m.mscid] = m
        ...
        if id_query:
            no_of_queries += 1
            sub_results_by_id = dict()
            for m in Scheme.search(Q.identifiers.any(Q.id == id_query)):
                sub_results_by_id[m.mscid] = m
            for m in Scheme.search(Q.versions.any(Q.identifiers.any(Q.id == id_query))):
                sub_results_by_id[m.mscid] = m
            flash_result(len(sub_results_by_id), f'with identifier "{id_query}"')
            results_by_id.update(sub_results_by_id)
        ...
        if funder_wild:
            ...
            for m in Group.search(Q.name.search(funder_query)):
            ...
            if funder_mscids:
                ...
                for f in funder_mscids:
                    for m in rel.subject_records(
                        ...
                        sub_results_by_id[m.mscid] = m
                flash_result(
                    len(sub_results_by_id), f'with funder matching "{funder_wild}"'
                )
                results_by_id.update(sub_results_by_id)
            else:
                flash(f'No funders found matching "{funder_wild}".', "error")
        ...
        document_list = sorted(results_by_id.values(), key=lambda k: k.name.lower())
        # Show results list
    ...
    if form.errors:
        if "csrf_token" in form.errors.keys():
            ...
            msg = (
                "Could not perform search as there {:/was an error/were N"
    ...
    # Sort suggestions:
    title_list = sorted(title_set, key=lambda k: k.lower()) if title_set else list()
    n = len(mscid_prefix) + 1
    id_list = sorted(id_set, key=lambda k: k[:n] + k[n:].zfill(5)) if id_set else list()
    funder_list = sorted(funder_set, key=lambda k: k.lower()) if funder_set else list()
    type_list = sorted(type_set, key=lambda k: k.lower()) if type_set else list()
    subject_list.sort()
    return render_template(
        "search-form.html",
        form=form,
...
def subject(subject: str):
    ...
    if no_of_hits:
        ...
        flash(
            ...
            ' Would you like to see some <a href="{}">generic schemes</a>?'.format(
                url_for_subject("Multidisciplinary")
            ),
            "error",
        )
    return render_template("search-results.html", title=subject, results=results)
...
def dataType(number: int):
    ...
    if no_of_hits:
        flash(
            "Found {:N scheme/s} used for this type of data.".format(
...
def group(role: t.Literal["funder", "maintainer", "user"], number: int):
    ...
    if no_of_hits:
        flash(
            "Found {:N scheme/s} {} by this organization.".format(
                Pluralizer(no_of_hits), verb
            )
        )
        results.sort(key=lambda k: k.name.lower())
    else:
        flash(f"No schemes found that are {verb} by this organization.", "error")
    return render_template("search-results.html", title=group.name, results=results)
