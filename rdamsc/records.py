                ...
                for predicate, objects in relation.items():
                    if isinstance(objects, list) and mscid in objects:
                        ...
                        if predicate in ["maintainers", "funders"]:
                            series = self.series_map.get(rel_mscid[mp_len : mp_len + 1])
...
class Record(Document, metaclass=ABCMeta):
    ...
    def cleanup(data: dict) -> dict:
        ...
        out the `csrf_token` and `old_relations` keys if present. Returns the
        result.
        """
    ...
    def get_choices(cls) -> t.List[t.Tuple[str, str]]:
        ...
        for record in cls.search(Query().slug.exists()):
            choices.append((record.mscid, record.name))
    ...
    def has_versions(self) -> bool:
        """Returns True if this class's data model includes versions,
        False otherwise."""
        return False

    @property
    def mscid(self) -> str:
        """The MSCID for this record, e.g. msc:m1."""
        return f"{mscid_prefix}{self.table}{self.doc_id}"

    @property
    def name(self) -> str:  # pragma: no cover
        """Name or title of this record (for display purposes)."""
        return "Generic record"

    @property
    def slug(self) -> str:
        """Filename-safe name or title of this record (for
    ...
    @abstractmethod
    def get_form(self) -> FlaskForm:  # pragma: no cover
        """Returns a FlaskForm instance for editing this record,
        populated with the current data.
        """
        raise NotImplementedError
    ...
    def insert_relations(self, data: t.Mapping) -> t.Mapping:
        ...
        for field in self.form():
            ...
            if field.flags.inverse:
                ...
                mscids.extend(
                    rel.subjects(
                        predicate=predicate, object=object, filter=field.flags.cls
                    )
                )
            else:
                subject = self.mscid
                mscids.extend(rel.objects(subject=subject, predicate=predicate))
    ...
    def reload(self):
        """Discards current data in this record, replacing it with the
        corresponding data stored in the database."""
        db = self.get_db()
        tb = db.table(self.table)
        ...
        for field in fields:
            ...
            if field.flags.inverse:
                ...
                old_subjects = old_relations.get(
                    field.name, rel.subjects(predicate=predicate, object=self.mscid)
                )
                ...
                old_objects = old_relations.get(
                    field.name, rel.objects(subject=self.mscid, predicate=predicate)
                )
                additions = list()
    ...
    def save_rel_record(
        ...
        indicates the field that triggered the error) and clean record.
        """
        ...
        for k, d in schema.items():
            ...
            if "schema" in d:
                ...
                for i, instance in enumerate(input_data[k]):
                    v_errors, v_value = self.validate_against(instance, d["schema"])
                    for error in v_errors:
                        errors.append(
                            {
                                "message": error.get("message", ""),
    ...
    def validate_rel_list(
        ...
        for i, mscid in enumerate(mscids):
            ...
            if rel_record is None:
                errors.append(
                    {"message": f"Not a valid MSC ID: {mscid}.", "location": f"[{i}]"}
                )
                continue
            elif rel_record.doc_id == 0:
                errors.append(
                    {"message": f"No such record: {mscid}.", "location": f"[{i}]"}
                )
                continue
    ...
    def validate_rel_patch(
        ...
        if op is None:
            errors.append(
                {"message": "JSON object must have an ‘op’ member.", "location": ""}
            )
        ...
        if path is None:
            errors.append(
                {"message": "JSON object must have a ‘path’ member.", "location": ""}
            )
        else:
                ...
                if op == "add":
                    if m.group("index") != "-":
                        if int(m.group("index")) > len(curr_list):
                            errors.append(
                                {
                                    "message": "Cannot add a value at that position.",
                                    "location": ".path",
                                }
                            )
                else:
                    if not curr_list:
                        errors.append(
                            {
                                "message": "No values exist at that position.",
                                "location": ".path",
                            }
                        )
                    elif m.group("index") != "-":
                        if int(m.group("index")) >= len(curr_list):
                            errors.append(
                                {
                                    "message": "No value exists at that position.",
                                    "location": ".path",
                                }
                            )
        ...
        if op == "test":
            ...
            if value != curr_value:
                errors.append(
                    {
                        "message": f"Test failed. Current value would be {curr_value}.",
                        "location": ".value",
                    }
                )
        elif op == "remove":
            if index is None:
                if m.group("predicate") not in output:
                    errors.append(
                        {"message": "Predicate already missing.", "location": ".path"}
                    )
                else:
            ...
            if index is None:
                ...
                if not isinstance(value, str):
                    ...
                    for error in list_errors:
                        errors.append(
                            {"message": error["message"], "location": ".value"}
                        )
                    if not list_errors:
            ...
            if index is None:
                if predicate not in output:
                    errors.append(
                        {"message": "Predicate needs to be added.", "location": ".path"}
                    )
...
class Scheme(Record):
    ...
    schema = {
        ...
        "namespaces": {"type": "namespaces", "optional": True},
        "identifiers": {"type": "identifiers", "useful": True},
        "relatedEntities": {"type": "relations"},
        "versions": {
            "schema": {
                "number": {"type": "versionid"},
                "title": {"type": "text"},
                "note": {"type": "html"},
                "issued": {"type": "date"},
                "available": {"type": "date"},
                ...
                "samples": {
                    "schema": {"url": {"type": "url"}, "title": {"type": "text"}}
                },
            }
        },
    }
    rolemap = {
        ...
        "user": {"predicate": "users", "direction": Relation.FORWARD, "accepts": "g"},
        "tool": {
    ...
    def get_form(self) -> "SchemeForm":
        ...
        # Populate form:
        form: SchemeForm = self.populate_form(data)
    ...
    def get_vform(self, index: int = None) -> "SchemeVersionForm":
        ...
        # Populate form:
        form: SchemeVersionForm = self.populate_form(data, is_version=True)
...
class Tool(Record):
    ...
    def get_vform(self, index: int = None) -> "ToolVersionForm":
        ...
        # Populate form:
        form: ToolVersionForm = self.populate_form(data, is_version=True)
...
class Crosswalk(Record):
    ...
    def name(self) -> str:
        ...
        rel = Relation()
        inputs = rel.object_records(subject=self.mscid, predicate="input schemes")
        outputs = rel.object_records(subject=self.mscid, predicate="output schemes")
    ...
    def get_form(self) -> "CrosswalkForm":
        ...
        # Populate form:
        form: CrosswalkForm = self.populate_form(data)
    ...
    def get_slug(self, *, apidata: t.Mapping = None, formdata: t.Mapping = None) -> str:
        ...
        if formdata:
            ...
            rel = Relation()
            inputs.extend(
                rel.object_records(subject=self.mscid, predicate="input schemes")
            )
            outputs.extend(
                rel.object_records(subject=self.mscid, predicate="output schemes")
            )
    ...
    def get_vform(self, index: int = None) -> "CrosswalkVersionForm":
        ...
        # Populate form:
        form: CrosswalkVersionForm = self.populate_form(data, is_version=True)
...
class Group(Record):
    ...
    def get_choices(cls):
        ...
        for scheme in cls.search(Query().slug.exists()):
            choices.append((scheme.mscid, scheme.get("name", "Unnamed")))
...
class Endorsement(Record):
    ...
    series = "endorsement"
    schema = {
        "title": {"type": "text", "or use role": "originator"},
        "description": {"type": "html", "optional": True},
        "creators": {
            "schema": {
                "fullName": {"type": "text"},
                "givenName": {"type": "text"},
        ...
        "publication": {"type": "text", "or use role": "originator"},
        "issued": {"type": "date", "or use": "valid"},
        "valid": {"type": "period", "or use": "issued"},
        "locations": {"type": "locations", "useful": True},
        "identifiers": {"type": "identifiers", "useful": True},
        "relatedEntities": {"type": "relations", "useful": ["endorsed scheme"]},
    }
    rolemap = {
        "endorsed scheme": {
            "predicate": "endorsed schemes",
    ...
    def get_form(self) -> "EndorsementForm":
        ...
        # Populate form:
        form: EndorsementForm = self.populate_form(data)
...
class Datatype(Record):
    ...
    table = "datatype"
    series = "datatype"
    schema = {"id": {"type": "url"}, "label": {"type": "text", "required": True}}
    ...
    @classmethod
    def load_by_label(cls, label: str) -> "Datatype":
        """Returns an instance of this class that corresponds to the
        given label, or a blank one if the label was not found in the
        database.
        """
        db = cls.get_db()
        tb = db.table(cls.table)
    ...
    def get_form(self) -> "DatatypeForm":
        # Get data from database:
        data = json.loads(json.dumps(self))

        # Populate form:
        form: DatatypeForm = self.form(data=data)
...
class VocabTerm(Document, metaclass=ABCMeta):
    ...
    @classmethod
    def populate(cls):
        """Populates the database with a starting set of terms
        (drawn from `data/vocabulary.json`).
        """
        db = cls.get_db()
        tables = db.tables()
        missing_tables = list()
        ...
        if missing_tables:
            ...
            data_file = os.path.join(moddir, "data", "vocabulary.json")
    ...
    def get_form(self) -> "VocabForm":
        ...
        # Populate form:
        form: VocabForm = self.form(data=data)
    ...
    def get_overlaps(self, id: str = None) -> t.List[str]:
        ...
        (i.e. if the list has members, an error would be introduced by
        saving this VocabTerm with the given ID).

        Otherwise, returns a list of Record series this VocabTerm
        cannot be set to apply to, because another VocabTerm with the
        same ID is already set to apply to it.
        """
        overlaps = list()
    ...
    def save_gui_input(self, formdata: t.Mapping) -> str:
        ...
        if overlaps:
            error = (
                f"A term with ID ‘{formdata['id']}’ has already been"
...
class EmailOrURL(object):
    ...
    def __init__(self, require_tld: bool = True):
        self.gen_regex = re.compile(r"^(?P<protocol>[a-z]+):.+", re.IGNORECASE)
    ...
    def __call__(self, form: Form, field: Field):
        ...
        if datum.startswith("mailto:"):
            ...
            if not match:
                raise ValidationError(
                    field.gettext("That email address does not look quite right.")
                )

        else:
            match = self.url_regex.match(datum)
            message = field.gettext("That URL does not look quite right.")
            if not match:
...
class NamespaceURI(object):
    ...
    def __call__(self, form: Form, field: Field):
        ...
        if not datum.endswith(("/", "#")):
            raise ValidationError(field.gettext('The URI must end with "/" or "#".'))

        match = self.url_regex.match(datum)
        message = field.gettext("That URI does not look quite right.")
        if not match:
            raise ValidationError(message)
        if not self.validate_hostname(match.group("host")):
            raise ValidationError(message)
    ...
    def __call__(self, form: Form, field: Field):
        ...
        for other_field_name in self.other_field_list:
            ...
            if other_field is None:
                raise Exception('No field named "{}" in form'.format(other_field_name))
...
class FormFieldFixed(FormField):
    ...
    def process(self, formdata: t.Mapping, data: t.Any = unset_value):
        ...
        if isinstance(data, dict):
            self.form = self.form_class(formdata=formdata, prefix=prefix, data=data)
        else:
...
class SelectRelatedField(SelectMultipleField):
    def __init__(
        ...
        **kwargs,
        ...
        super(SelectMultipleField, self).__init__(label, choices=choices, **kwargs)
...
class IdentifierForm(Form):
    ...
    scheme = SelectField("ID scheme", validators=[RequiredIf(["id"])], default="")
...
class NamespaceForm(Form):
    prefix = StringField(
        "Prefix", validators=[RequiredIf(["uri"]), validators.Length(max=32)]
    )
    uri = StringField("URL", validators=[RequiredIf(["prefix"]), NamespaceURI()])
...
class SchemeForm(FlaskForm):
    ...
    maintainers = SelectRelatedField(
        "Organizations that maintain this scheme", Group, description="maintainers"
    )
    funders = SelectRelatedField(
        "Organizations that funded this scheme", Group, description="funders"
    )
    users = SelectRelatedField(
        "Organizations that use this scheme", Group, description="users"
    )
    tools = SelectRelatedField(
        "Tools that support this scheme",
        Tool,
...
class SchemeVersionForm(FlaskForm):
    number = StringField("Version number", validators=[validators.Length(max=32)])
    title = StringField("Name of metadata scheme")
    ...
    namespaces = FieldList(
        FormFieldFixed(NamespaceForm),
        "Predicate namespaces for this version of the scheme",
        min_entries=1,
    )
    identifiers = FieldList(
        FormField(IdentifierForm),
        "Identifiers for this version of the scheme",
        min_entries=1,
    )
    samples = FieldList(
        FormField(SampleForm), "Sample records conforming to this scheme", min_entries=1
    )
...
class ToolForm(FlaskForm):
    ...
    description = TextHTMLField("Description")
    types = SelectMultipleField("Type of tool", render_kw={"size": 5})
    ...
    creators = FieldList(
        FormField(CreatorForm), "People responsible for this tool", min_entries=1
    )
    ...
    maintainers = SelectRelatedField(
        "Organizations that maintain this tool", Group, description="maintainers"
    )
    funders = SelectRelatedField(
        "Organizations that funded this tool", Group, description="funders"
    )
    old_relations = HiddenField()
...
class CrosswalkForm(FlaskForm):
    ...
    identifiers = FieldList(
        FormField(IdentifierForm), "Identifiers for this mapping", min_entries=1
    )
    creators = FieldList(
        FormField(CreatorForm), "People responsible for this mapping", min_entries=1
    )
    input_schemes = SelectRelatedField(
        "Input metadata scheme(s)", Scheme, description="input schemes"
    )
    output_schemes = SelectRelatedField(
        "Output metadata scheme(s)", Scheme, description="output schemes"
    )
    maintainers = SelectRelatedField(
        "Organizations that maintain this mapping", Group, description="maintainers"
    )
    funders = SelectRelatedField(
        "Organizations that funded this mapping", Group, description="funders"
    )
    old_relations = HiddenField()
...
class GroupForm(FlaskForm):
    ...
    locations = FieldList(FormField(LocationForm), "Relevant links", min_entries=1)
    identifiers = FieldList(
        FormField(IdentifierForm), "Identifiers for this organization", min_entries=1
    )
    maintained_schemes = SelectRelatedField(
        "Schemes maintained by this organization",
        Scheme,
        description="maintainers",
        inverse=True,
    )
    maintained_tools = SelectRelatedField(
        "Tools maintained by this organization",
        Tool,
        description="maintainers",
        inverse=True,
    )
    maintained_mappings = SelectRelatedField(
        "Mappings maintained by this organization",
        Crosswalk,
        description="maintainers",
        inverse=True,
    )
    funded_schemes = SelectRelatedField(
        "Schemes funded by this organization",
        Scheme,
        description="funders",
        inverse=True,
    )
    funded_tools = SelectRelatedField(
        "Tools funded by this organization", Tool, description="funders", inverse=True
    )
    funded_mappings = SelectRelatedField(
        "Mappings funded by this organization",
        Crosswalk,
        description="funders",
        inverse=True,
    )
    used_schemes = SelectRelatedField(
        "Schemes used by this organization", Scheme, description="users", inverse=True
    )
    endorsements = SelectRelatedField(
...
class EndorsementForm(FlaskForm):
    ...
    creators = FieldList(
        FormField(CreatorForm), "Authors of the endorsement document", min_entries=1
    )
    ...
    identifiers = FieldList(
        FormField(IdentifierForm), "Identifiers for this endorsement", min_entries=1
    )
    endorsed_schemes = SelectRelatedField(
        "Endorsed schemes", Scheme, description="endorsed schemes"
    )
...
class DatatypeForm(FlaskForm):
    id = StringField(
        "URL identifying this type of data", validators=[Optional(), EmailOrURL()]
    )
    label = StringField(
        "Descriptor for this type of data", validators=[validators.InputRequired()]
    )


class VocabForm(FlaskForm):
    id = StringField("Database value", validators=[validators.Length(max=64)])
    label = StringField("Displayed value", validators=[validators.InputRequired()])
...
# Utility functions
# =================
def get_data_db() -> TinyDB:
    """Returns the main database as a TinyDB object. The object is
...
def get_term_db() -> TinyDB:
    """Returns the terms database as a TinyDB object. The object is
    cached so further calls return the same one.
    """
    if "term_db" not in g:
        g.term_db = TinyDB(
...
def get_safe_pad() -> int:
    """Returns the smallest power of ten (but at least 100) that is
    larger than the number of records in the largest table in the
    main database. Useful for calculating a global sort order.
    """
...
def strip_tags(string: str) -> str:
    ...
    # Check matched tags:
    matches = re.finditer(
        r"<(?P<close>/)?(?P<tag>\w+)(?P<attrib> [^>]*)?>", string, re.DOTALL
    )
    seen = list()
    for m in matches:
...
def edit_record(table: MainTableID, number: int):
    ...
    if record.doc_id != number:
        flash(
            ...
            "Try filling out this new one instead.",
            "error",
        )
    ...
    if request.method == "POST" and form.validate():
        ...
        if error:
            flash(error, "error")
            return redirect(url_for("main.edit_record", table=table, number=number))
        else:
            if number:
            ...
            return redirect(url_for("main.display", table=table, number=record.doc_id))
    elif record.has_versions:
        flash(
            "Fill out information here that applies to all versions."
    ...
    if form.errors:
        if "csrf_token" in form.errors.keys():
            ...
            msg = (
                "Could not save changes as there {:/was an error/were N"
...
def edit_version(table: MainTableID, number: int, index: int = None):
    ...
    if index is not None and index >= len(record.get("versions", list())):
        flash(
            ...
            "Try adding this new one instead.",
            "error",
        )
        return redirect(url_for("main.edit_version", table=table, number=number))
    ...
    if request.method == "POST" and form.validate():
        ...
        if index is None:
            ...
            if error:
                ...
                return redirect(
                    url_for(
                        "main.edit_version", series=table, number=number, index=index
                    )
                )
            else:
...
@bp.route(
    "/edit/<any(datatype, location, type, id_scheme):vocab><int:number>",
    methods=["GET", "POST"],
)
@login_required
def edit_vocabterm(
...
def display(table: MainTableID, number: int, field: str = None):
        ...
        for v in record["versions"]:
            ...
            if "issued" in v:
                ...
                if "valid" in v:
                    ...
                    if valid_end:
                        this_version["status"] = "deprecated on {}".format(valid_end)
                    else:
    ...
    for field in record.form():
        if field.type != "SelectRelatedField":
            continue
        if field.description in ["parent schemes", "input schemes", "output schemes"]:
            scheme_scheme_fields.append(field.name)
        if field.flags.inverse:
            others = rel.subject_records(
                predicate=field.description, object=record.mscid, filter=field.flags.cls
            )
        else:
    ...
    if table == "m":
        ...
        params["hasRelatedGroups"] = (
            "maintainers" in relations or "funders" in relations or "creators" in record
        )
    elif table == "g":
        records_seen = {"schemes": dict(), "tools": dict(), "mappings": dict()}
        relrec = dict()
        relids = dict()
