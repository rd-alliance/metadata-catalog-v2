...
def test_record_index(client: FlaskClient, page: PageActions, data_db: DataDBActions):
    ...
    page.assert_contains(
        "  " * 5 + '<a class="nav-link" href="/msc/m2">Test scheme 2</a>'
    )
    ...
    page.assert_contains(
        "  " * 3 + '<a class="nav-link" href="/msc/t1">Test tool 1</a>'
    )
    ...
    page.read(html)
    page.assert_contains(
        "  " * 3 + '<a class="nav-link" href="/msc/g1">Organization 1</a>'
    )
...
def test_bad_record_index(
    ...
    page.assert_contains(
        "  " * 7 + '<a class="nav-link" href="/msc/m3">Test scheme 3</a>'
    )
...
def test_subject_index(client: FlaskClient, page: PageActions, data_db: DataDBActions):
    ...
    html = response.get_data(as_text=True)
    page.read(html)
    page.assert_contains('<a class="nav-link" href="/subject/Science">Science</a>')
    page.assert_contains(
