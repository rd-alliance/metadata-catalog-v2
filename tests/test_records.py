...
def test_create_view_records(
    ...
    html = response.get_data(as_text=True)
    page.read(html)
    page.assert_contains("You are trying to update a record that doesn't exist.")
    page.assert_contains("Add new metadata scheme")
    ...
    m2["description"] = m2["description"].replace(
        "1.</p>", '<span class="mso-nasty">1</span><script>do_evil();</script>.</p>'
    )
...
def test_create_terms(
    ...
    response = client.post("/edit/datatype0", data=datatype1, follow_redirects=True)
    html = response.get_data(as_text=True)
    page.assert_contains("Successfully added record.", html)
    ...
    loc2json = (
        '{ "id": "website", "label": "homepage","applies": ["scheme", "mapping"]}'
    )
    ...
    page.assert_contains("A term with ID ‘website’ has already been coined for scheme.")
    ...
    loc4json = (
        '{ "id": "document", "label": "documentation","applies": ["scheme", "tool"]}'
    )
